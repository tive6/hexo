...
因此，Markdown被广泛的应用于`hexo个人博客`、`Github`、`Coding`等具有大量文本的交流学习网站。

<!--more-->

![Markdown](http://upload-images.jianshu.io/upload_images/3876828-3a7ee99d25498159.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

以下就是Markdown中常用的格式写法：
...
* 原文地址：http://zmnaer.com/tags/Markdown/

*提示：*

* 一般Markdown文档都是以`.md`后缀结尾的；
* `Github`、`Coding`等开源社区，在仓库的根目录下一般都存在`README.md`文件，它就是Markdown格式编写的，可以直接在仓库的下方展现出来，用于仓库的说明、或API的概述；
* 文中所有的`·`都是`英文输入法`下`Tab键`上边的那个字符，`+`号请忽略不要写。（补充：高亮显示和特别显示中的`·`）；
